...
mod tests {
    ...
    // Windows links an externally installed library instead of the vendored build.
    #[cfg(not(target_os = "windows"))]
    #[test]
    fn vendored_runtime_version() {
        assert_eq!(version().split_whitespace().next(), Some("5.4.13"));
        assert_eq!(kingfisher_vectorscan_sys::HS_PATCH, 13);
        assert_eq!(
            kingfisher_vectorscan_sys::HS_VERSION_32BIT,
            (5 << 24) | (4 << 16) | (13 << 8)
        );
    }

    #[test]
    fn unsupported_character_class_operations_are_rejected() {
        // Exercise the 5.4.13 grammar through the checked-in Ragel output.
        for expression in [b"[a[b]c]".as_slice(), b"[a-z&&[^aeiou]]"] {
            let pattern = Pattern::new(expression.to_vec(), Flag::default(), None);
            assert!(BlockDatabase::new(vec![pattern]).is_err());
        }
        let literal_bracket = Pattern::new(br"[a\[]".to_vec(), Flag::default(), None);
        assert!(BlockDatabase::new(vec![literal_bracket]).is_ok());
    }
